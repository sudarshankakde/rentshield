...
@Component({
  ...
  template: `
    <div class="max-w-6xl mx-auto space-y-8 pb-12">
      <section class="bg-surface border border-muted p-8 shadow-sm">
        <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div class="max-w-2xl">
            <p class="text-xs uppercase tracking-[0.28em] text-brand font-black">Home</p>
            <h1 class="mt-5 text-4xl font-black tracking-tight">Your dashboard for {{ roleLabel }}</h1>
            <p class="mt-4 text-muted-var leading-7">Each module below is a self-contained workflow. Tap the card for the feature your role should use today.</p>
          </div>
          <div class="border border-muted bg-surface-soft p-6 shadow-sm">
            ...
            <p class="mt-2 text-sm text-muted-var">Signed in as {{ auth.user()?.name || 'User' }}</p>
          </div>
        </div>
      </section>
        ...
        <ng-container *ngIf="!uiConfig.loading()">
          <ng-container *ngIf="roleModules().length; else emptyState">
            <a *ngFor="let module of roleModules()" [routerLink]="['/', module.path]"
               class="group border border-muted bg-surface p-6 shadow-sm transition hover:shadow-md hover:border-brand">
              <div class="flex items-center justify-between gap-3">
                <div class="w-12 h-12 flex items-center justify-center text-white" [ngClass]="module.color">
              ...
              <h2 class="mt-5 text-lg font-black">{{ module.name }}</h2>
              <p class="mt-2 text-sm leading-6 text-muted-var">{{ module.description }}</p>
              <div class="mt-5 flex items-center justify-between border-t border-muted pt-4">
                <span class="text-xs uppercase text-brand font-black">Open</span>
                <lucide-icon [name]="ArrowIcon" size="16" class="text-brand"></lucide-icon>
              </div>
            </a>
          </ng-container>
