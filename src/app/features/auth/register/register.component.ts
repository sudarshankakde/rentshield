...
import { LucideAngularModule, ArrowRight, Lock, Mail, User } from 'lucide-angular';
...
@Component({
  ...
  template: `
    <app-auth-layout title="Create account" subtitle="Register your profile and role to unlock module access.">
      <form class="auth-form" [formGroup]="form" (ngSubmit)="submit()" novalidate>
        <div class="grid grid-cols-2 gap-3">
          <div class="input-group">
            <label class="input-label">First name</label>
            <div class="input-wrap">
              <span class="input-icon"><lucide-icon [name]="UserIcon" size="16"></lucide-icon></span>
              <input class="input-field pl-10" formControlName="firstName" autocomplete="given-name" placeholder="John">
            </div>
            <p *ngIf="showError('firstName')" class="input-error">First name is required.</p>
          </div>
          ...
          <div class="input-group">
            ...
            <div class="input-wrap">
              <span class="input-icon"><lucide-icon [name]="UserIcon" size="16"></lucide-icon></span>
              <input class="input-field pl-10" formControlName="lastName" autocomplete="family-name" placeholder="Doe">
            </div>
            <p *ngIf="showError('lastName')" class="input-error">Last name is required.</p>
          </div>
        </div>
        ...
        <div class="input-group">
          ...
          <div class="input-wrap">
            ...
            <input class="input-field pl-10" type="password" formControlName="password" placeholder="At least 6 characters" autocomplete="new-password">
          </div>
        ...
        <div class="input-group">
          ...
          <div class="grid grid-cols-3 gap-2 mt-1.5">
            <button *ngFor="let option of roles" type="button" class="role-pill" [class.role-pill--active]="form.controls.role.value === option.value" (click)="form.controls.role.setValue(option.value)">
              {{ option.label }}
            </button>
          </div>
        </div>
...
export class RegisterComponent {
  ...
  readonly form = this.fb.nonNullable.group(
    {
      ...
      role: this.fb.nonNullable.control<UserRole>('TENANT', Validators.required),
    },
    {
      validators: (group) => {
  ...
  showError(controlName: 'firstName' | 'lastName' | 'email' | 'password'): boolean {
